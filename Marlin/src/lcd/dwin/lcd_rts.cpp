...
int heatway = 0;
millis_t next_rts_update_ms      = 0; //刷新ui的时间间隔
...
int temphot = 0;  //保存暂停前喷头的目标温度
bool rts_start_print = false;  //防止第二次空文件名打印

/*******************************本地全局变量定义*********************************/
/*************************************END***************************************/

//========================================================================
// 函数: RTS_line_to_current(AxisEnum axis) 
// 描述: 插入命令路径规划缓冲队列
// 参数: AxisEnum axis 所需要移动/控制的轴
// 返回: 无
// 版本: VER1.0
// 日期: 2021-7-14
// 备注: 
...
//========================================================================
// 函数: bool RTSSHOW::RTS_SD_Detected(void)
// 描述: 检测SD卡是否插入
// 参数: void
// 返回: 无
// 版本: VER1.0
...
//========================================================================
// 函数: void RTSSHOW::RTS_SDCardInit(void)
// 描述: 初始化SD信息
// 参数: void
// 返回: 无
// 版本: VER1.0
...
//========================================================================
// 函数: void RTSSHOW::RTS_SDCardUpate(void)
// 描述: 刷新读取SD的信息
// 参数: void
// 返回: 无
// 版本: VER1.0
...
void RTSSHOW::RTS_Init(void)
{
  ...
  RTS_SndData( thermalManager.fan_speed[0] , PRINTER_FAN_SPEED_DATA_VP);  //打印风扇速度初始化
  ...
  for(startprogress = 0; startprogress <= 100; startprogress++)
  {
    ...
    HAL_watchdog_refresh();  //喂狗否则会超时复位
    delay(100);
  }
...
//========================================================================
// 函数: int RTSSHOW::RTS_RecData(void)
// 描述: 接收串口的数据并保存到相应的数组中
// 参数: void
// 返回: 无
// 版本: VER1.0
// 日期: 2021-7-14
...
//========================================================================
// 函数: void RTSSHOW::RTS_SndData(void)
// 描述: 发送控制数据到串口屏中
// 参数: void
// 返回: 无
// 版本: VER1.0
...
//========================================================================
// 函数: void RTSSHOW::RTS_SndData(const String &s, unsigned long addr, unsigned char cmd /*= VarAddr_W*/)
// 描述: 发送对应地址的串口数据控制串口屏的相应组件
// 参数: const String &s, unsigned long addr, unsigned char cmd /*= VarAddr_W*/ 数据，地址，指令
// 返回: 无
// 版本: VER1.0
// 日期: 2021-7-14
// 备注:
...
//========================================================================
// 函数: void RTSSHOW::RTS_SndData(const char *str, unsigned long addr, unsigned char cmd/*= VarAddr_W*/)
// 描述: 串口数据发送函数
// 参数: const char *str, unsigned long addr, unsigned char cmd/*= VarAddr_W*/
// 返回: 无
// 版本: VER1.0
// 日期: 2021-7-14
// 备注:
...
//========================================================================
// 函数: void RTSSHOW::RTS_SndData(char c, unsigned long addr, unsigned char cmd /*= VarAddr_W*/)
// 描述: 串口数据发送函数，发送单个字符
// 参数: char c, unsigned long addr, unsigned char cmd
// 返回: 无
// 版本: VER1.0
// 日期: 2021-7-14
// 备注:
...
//========================================================================
// 函数: void RTSSHOW::RTS_SndData(int n, unsigned long addr, unsigned char cmd /*= VarAddr_W*/)
// 描述: 串口数据发送函数，发送寄存器数据
// 参数: char c, unsigned long addr, unsigned char cmd
// 返回: 无
// 版本: VER1.0
...
//========================================================================
// 函数: void RTSSHOW::RTS_SndData(unsigned long n, unsigned long addr, unsigned char cmd /*= VarAddr_W*/)
// 描述: 发送串口数据
// 参数: void
// 返回: 无
// 版本: VER1.0
...
//========================================================================
// 函数: void RTSSHOW::RTS_SDcard_Stop(void)
// 描述: 停止SD卡打印文件
// 参数: void
// 返回: 无
// 版本: VER1.0
...
//========================================================================
// 函数: void RTSSHOW::RTS_HandleData(void)
// 描述: 处理串口的显示逻辑
// 参数: void
// 返回: 无
// 版本: VER1.0
...
void RTSSHOW::RTS_HandleData(void)
{
  ...
  {
    if(recdat.addr == Addrbuf[i])     //判断串口接收到的地址是第几页的，然后赋值给Checkkey，进行相应的页面跳转
    {
  ...
  switch(Checkkey)
  {
    case MainEnterKey:        //跳转到相应的页面,特指按钮的作用
      if(recdat.data[0] == 1)   //判断对应地址的键值是多少
      {
        CardUpdate = true;
      ...
      {
        // card.flag.abort_sd_printing = true;  //停止后机器会再次回原点
        queue.clear();
        quickstop_stepper();
        print_job_timer.stop();
        RTS_SndData(1, MOTOR_FREE_ICON_VP);
    ...
    case PrepareEnterKey:
      ...
      else if(recdat.data[0] == 0xB)
      {
        ...
        #endif
        (void)settings.reset();  //重置eeprom里面的数据
        (void)settings.save();
        RTS_SndData(ExchangePageBase + 33, ExchangepageAddr);
        change_page_font = 33;
      }
...
//========================================================================
// 函数: void RTSSHOW::languagedisplayUpdate(void)
// 描述: 多语言切换控制函数
// 参数: void
// 返回: 无
// 版本: VER1.0
...
//========================================================================
// 函数: void RTS_PauseMoveAxisPage(void)
// 描述: 跳转暂停后移动轴
// 参数: void
// 返回: 无
// 版本: VER1.0
...
//========================================================================
// 函数: void RTS_AutoBedLevelPage(void)
// 描述: 自动调平界面
// 参数: void
// 返回: 无
// 版本: VER1.0
...
//========================================================================
// 函数: void RTS_MoveAxisHoming(void)
// 描述: 回原点界面
// 参数: void
// 返回: 无
// 版本: VER1.0
