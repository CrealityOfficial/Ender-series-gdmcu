...
void menu_main() {
    ...
    auto sdcard_menu_items = [&]{
      ...
      if (card_detected) {
        if (!card_open) {
          ...
          #endif
          if (!busy) SUBMENU(MSG_MEDIA_MENU, MEDIA_MENU_GATEWAY);        // Media Menu (or Password First)
        }
      }
      else {
        #if PIN_EXISTS(SD_DETECT)
  ...
  if (busy) {
      ...
      if (thermalManager.temp_bed.celsius >= (thermalManager.temp_bed.target-5))
      {
       if((thermalManager.temp_hotend[0].celsius >= (thermalManager.temp_hotend[0].target - 5)) && (planner.has_blocks_queued()))
       {
