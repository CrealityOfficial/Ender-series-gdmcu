...
//
  ...
  void menu_info_printer() {
    ...
    STATIC_ITEM_P(PSTR(MACHINE_NAME));                          // My 3D Printer
    STATIC_ITEM_P(PSTR(SOFTWARE_VERSION));                      // software x.x.x
    STATIC_ITEM_P(PSTR(MAIN_BOARD_VERSION));                    // mainboard x.x.x
    STATIC_ITEM_P(PSTR(SHORT_BUILD_VERSION));                   // Marlin 2.0.x
    STATIC_ITEM_P(PSTR(STRING_DISTRIBUTION_DATE));              // YYYY-MM-DD HH:MM
    STATIC_ITEM_P(PSTR(WEBSITE_URL));                           // www.creality.com
    STATIC_ITEM_P(PSTR("Extruders:" STRINGIFY(EXTRUDERS)));     // Extruders: 1
    STATIC_ITEM_P(PSTR("Baudrate:" STRINGIFY(BAUDRATE)), SS_CENTER);
